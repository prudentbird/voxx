...
async function listPostsCached(
  ...
  cacheLife("max");
  void version;
  return coreListPosts({ ...{ collection: "blog" }, ...opts });
}
...
async function listReachablePostsCached(version: number): Promise<PostMeta[]> {
  ...
  cacheLife("max");
  void version;
  const { posts } = await coreListPosts({ ...{ collection: "blog" }, reachable: true });
  return posts;
}
...
async function getPostsPageCached(
  ...
  cacheLife("max");
  void version;
  return coreGetPosts({ ...{ collection: "blog" }, offset, limit });
}
...
async function getPostCached(
  ...
  void version;
  try {
    return await coreGetPost(slug, { ...{ collection: "blog" }, reachable: true });
  } catch {
    return null;
  }
}
